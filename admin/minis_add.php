...
if(isset($_POST['btnInsert'])){
    ...
    @$m_impo = $_POST['m_impo'];
		...
		$sql ="SELECT * FROM ministry  WHERE  m_name='$m_name'  ";
		$num = dbNumRows(dbQuery($sql));
			if ($num>0){
				echo "<script>
              swal({
                    title: 'ชื่อสังกัดซ้ำ!',
        ...
        exit();
          ...
          $sql = "INSERT INTO ministry(m_impo,m_name)  VALUES ($m_impo,'$m_name')";
          echo $sql;
          $result=dbQuery($sql);
          if($result){
...
if(isset($_POST['btnEdit'])){
  ...
  $sql = "UPDATE ministry SET m_impo = $m_impo, m_name = '$m_name' WHERE m_id = $m_id";
...
if(isset($_GET['m_id'])){
   $m_id=$_GET['m_id'];
   $sql = "DELETE FROM ministry WHERE m_id = $m_id";
   $result = dbQuery($sql);
   if($result){
     echo "<script>swal('เรียบร้อย!','บันทึกการเปลี่ยนแปลงแล้ว','success');</script>";
   }else{
