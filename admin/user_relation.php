...
if (isset($_GET['delItem'])) {
  $res_id = intval($_GET['delItem']);
  $sql = "DELETE FROM respon WHERE res_id = ?";
  $result = dbQueryPrepared($sql, [$res_id]);
  if ($result) {
    echo "<script>
            Swal.fire('เรียบร้อย!', 'ลบสิทธิ์เรียบร้อยแล้ว!', 'success').then(() => {
                window.location.replace('user_relation.php');
            });
        </script>";
  }
}
...
if (isset($_POST['recal'])) {
  ...
  if ($res_dep != '0' && !empty($res_user)) {
    // Check for existing
    $chd_sql = "SELECT * FROM respon WHERE res_user = ? AND res_dep = ?";
    $chd_result = dbQueryPrepared($chd_sql, [$res_user, $res_dep]);
    if (dbNumRows($chd_result) > 0) {
      echo "<script>Swal.fire('คำเตือน', 'ผู้ใช้คนนี้มีสิทธิ์ดูแลหน่วยงานนี้แล้ว', 'warning');</script>";
    } else {
      $sql = "INSERT INTO respon (res_user, res_dep) VALUES (?, ?)";
      $result = dbQueryPrepared($sql, [$res_user, $res_dep]);
      if ($result) {
        echo "<script>
                    Swal.fire('เรียบร้อย!', 'ผูกสิทธิ์การใช้งานเรียบร้อยแล้ว!', 'success').then(() => {
                        window.location.replace('user_relation.php');
                    });
                </script>";
      }
            ...
            <form action="user_relation.php" method="post" name="form1">
              ...
              <div class="form-group">
                <div class="input-group">
                  ...
                  <select class=" selectpicker form-control" data-live-search="true" title="โปรดระบุ" name="u_dep"
                    id="u_dep">
              ...
              <div class="form-group">
                <div class="input-group">
                  ...
                  <select class=" selectpicker form-control" data-live-search="true" title="โปรดระบุ" name="u_name"
                    id="u_name">
    ...
    <div class="col-md-12">
      <div class="card">
          ...
          <div class="float-right">
            <form method="get" class="form-inline">
              ...
              <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" name="ser" id="ser_u" value="u" <?php if (!isset($_GET['ser']) || $_GET['ser'] == 'u')
                  echo 'checked'; ?>>
                <label class="form-check-label" for="ser_u">User</label>
              </div>
              <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" name="ser" id="ser_d" value="d" <?php if (isset($_GET['ser']) && $_GET['ser'] == 'd')
                  echo 'checked'; ?>>
        ...
        <div class="card-body">
          ...
          ?>
          <?php include("../code/show_relation.php"); ?>
        </div> <!-- card-body -->
      </div>
    </div> <!-- col -->
  </div> <!-- row -->
