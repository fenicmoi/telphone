...
if (isset($_POST['btnSearch'])) {
	$minis = $_POST['minis'];
	$m_sql = "SELECT * FROM ministry WHERE m_id = ?";
	$m_res = dbQueryPrepared($m_sql, [$minis]);
	$presee_row = dbFetchArray($m_res);
	$m_name = $presee_row ? $presee_row["m_name"] : "ไม่พบข้อมูล";
} else {
	$minis = "12";
	...
	<form action="head_add.php" method="post">
		<div class="form-group">
			<div class="input-group">
				...
				<button type="submit" name="btnSearch" id="btnSearch" class="btn btn-info"><i class="fas fa-filter"></i>
					กรอง</button>
...
<table class="table table-bordered" id="manager">
	<thead>
		...
		$s_res = dbQueryPrepared($s_sql, [$minis]);
		$s_row = dbFetchArray($s_res);
		...
		if ($d_num > 0) {
			...
			echo "<tr><td colspan=990><div align=center><strong><FONT COLOR=red>ไม่มีข้อมูล</FONT></strong></div></td></tr>";
		}
		?>
	</tbody>
	</thead>
