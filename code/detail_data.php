...
$g_id = $_GET['g_id'];
$g_sql = "SELECT * FROM govern WHERE g_id = ?";
$g_result = dbQueryPrepared($g_sql, [$g_id]);
...
if ($g_num > 0) {
	while ($g_row = dbFetchArray($g_result)) {
		...
		$d_sql = "SELECT * FROM depart WHERE dep_id = ?";
		$d_res = dbQueryPrepared($d_sql, [$g_dep]);
		$d_row = dbFetchArray($d_res);
		$d_name = $d_row["dep_name"];
		$d_minis = $d_row["dep_minis"];

		$s_sql = "SELECT * FROM ministry WHERE m_id = ?";
		$s_res = dbQueryPrepared($s_sql, [$d_minis]);
