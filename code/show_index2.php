<table class="table table-bordered">
    ...
    while ($s_row = dbFetchArray($s_result)) {
        ...
        $d_sql = "SELECT * FROM depart WHERE dep_minis = ? AND dep_name LIKE ? ORDER BY dep_impo ASC";
        $d_result = dbQueryPrepared($d_sql, [$s_id, "%$key_w%"]);
        ...
        if ($d_num > 0) {
            ...
            while ($d_row = dbFetchArray($d_result)) {
                ...
                <tr height=25 class='bg-info'>
                    <td colspan="2" width="30%"><strong>
                            <div align=left><?php echo $d_name; ?>
                        </strong></div>
                    </td>
                ...
                if ($g_num > 0) {
                    while ($g_row = dbFetchArray($g_result)) {
                        ...
                        <tr valign=top bgcolor=#FFFFFF>
                            ...
                            <td>
                                <div align=left> &nbsp;&nbsp;<B>
                                        <FONT COLOR=006600><?= $g_position ?></FONT>
                                    </B> <BR>
                                    &nbsp;&nbsp; <a href="" data-toggle="modal" data-target="#modelDetail"
                                        onclick="load_data('<?php echo $g_id; ?>');"><?= $g_head_th; ?> </a>
                                    <BR><BR>
                                </div>
                            </td>
                            <td>
